...
/**
 * 이용약관·개인정보처리방침·환불 및 취소 정책 원문(큐 Q25 2차). 고객용 /[locale]/terms · /[locale]/privacy 가 읽는다.
 * 종류+언어당 한 행. 행이 없으면 화면은 "준비 중"을 보여준다 — 받지 못한 문서를 지어내 채우지 않는다.
 * 수정은 최고관리자만. 지우지 않는다(동의 근거가 사라진다). 고칠 때마다 legal-revisions 에 남는다.
 */
const LABEL: Record<string, string> = { terms: '이용약관', privacy: '개인정보처리방침', refund: '환불 및 취소 정책' }
...
export const LegalDocuments: CollectionConfig = {
  ...
  hooks: { afterChange: [recordLegalRevision('legal-documents', (doc) => `${LABEL[doc.kind as string] ?? '이용약관'} (${doc.locale})`)] },
  fields: [
    { name: 'kind', type: 'select', required: true, options: ['terms', 'privacy', 'refund'], index: true },
