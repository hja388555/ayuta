...
import { z } from 'zod'
import { adminThreadItems, chatGate, findThreadById, jsonError, listMessages, messageView, parseAfter, parseId, readJson, sendMessage } from '@/lib/chat/service'
import { MAX_BODY } from '@/lib/chat/rules'

type Ctx = { params: Promise<{ id: string }> }
const BodySchema = z.object({ body: z.string().min(1).max(MAX_BODY) }).strict()

/** 관리자: 방의 메시지(after 가 없으면 최근 50개). thread 에는 목록과 같은 머리글 정보(이름·비회원 여부)를 함께 준다 */
export async function GET(req: Request, { params }: Ctx): Promise<Response> {
  ...
  const [item] = await adminThreadItems(g.payload, [thread])
  return NextResponse.json({ thread: item, messages: messages.map((m) => messageView(m, true)) })
}
...
export async function POST(req: Request, { params }: Ctx): Promise<Response> {
  ...
  const thread = await findThreadById(g.payload, id)
  if (!thread) return jsonError('not_found', 404)
  const r = await sendMessage(g.payload, thread, 'admin', { userId: g.user.id, email: g.user.email }, parsed.data.body)
