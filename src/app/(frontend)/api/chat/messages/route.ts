...
import { z } from 'zod'
import { customerGate, customerThread, jsonError, listMessages, messageView, parseAfter, readJson, sendMessage } from '@/lib/chat/service'
import { MAX_BODY } from '@/lib/chat/rules'

const BodySchema = z.object({ body: z.string().min(1).max(MAX_BODY) }).strict()

/** 고객(회원·비회원): 내 방의 새 메시지(폴링). 방이 없으면 404 */
...
/** 고객(회원·비회원): 내 방에 메시지 보내기. 회원 방은 GET /api/chat/thread 가 먼저 만든다 */
export async function POST(req: Request): Promise<Response> {
  ...
  const thread = await customerThread(g)
  if (!thread) return jsonError('not_found', 404)
  const r = await sendMessage(g.payload, thread, 'customer', g.sender, parsed.data.body)
